...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "default", ...props }, ref) => {
    ...
    const variantClasses = {
      ...
      outline: isDark 
        ? "border border-[#444444] bg-transparent hover:bg-[#3A3A40] text-white" 
        : "border border-gray-300 bg-transparent hover:bg-gray-100 text-black",
      ghost: isDark 
        ? "bg-transparent hover:bg-[#3A3A40] text-white" 
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  const { onSend, onSendMessage, isLoading = false, placeholder = "Type your message here...", className } = props;
  const { theme } = useTheme();
  const { user } = useAuth();
  ...
  const handleStopRecording = (duration: number) => {
    ...
    const voiceMessage = `[Voice message - ${duration} seconds]`;
    ...
    <>
      <PromptInput
        ...
        className={cn(
          ...
          isDark 
            ? "bg-card/90 border-border shadow-[0_8px_30px_rgba(0,0,0,0.24)]" 
            : "bg-card/95 border-border shadow-[0_4px_20px_rgba(0,0,0,0.08)]",
